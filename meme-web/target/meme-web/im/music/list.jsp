...
$(function(){
	...
	datagrid = $('#grid').datagrid({
        ...
        columns: [[//显示的列
        	 { field: 'contentid', title: '编号', checkbox: true},
        	 { field: 'columnid', title: '所属项目编号', hidden: true},
        	 { field: 'catid', title: '音乐类型', hidden: true},
        	 
             { field: 'contentname', title: '歌名', width: 100, 
                 editor: { type: 'validatebox', options: { required: true} }
             },
             { field: 'author', title: '歌手名', width: 100, 
                 editor: { type: 'validatebox', options: { required: true} }
             },
               ...
               { field: 'lrc', title: '歌词地址', width: 100, 
                   editor: { type: 'validatebox', options: { required: true} }
               },
		...
		for (var i = 0, len = editors.length; i < len; i++) {
			...
			$(editor.target).bind('keyup',function(e) {
								...
								if (code == 13) {
									...
									if (checkData(rows[0])) {
										$.ajax({
							url : '<%=basePath%>im/music/doEdit.do?account'+acc,
							type: 'post',
							dataType: 'json',
							data:rows[0],
    ...
    if (editRow == undefined) {
    	...
    	$(editor.target).bind('keyup', function (e) {
    	    ...
    	    if (code == 13) {
    	        ...
    	        if(checkData(rows[0])) {
    				$.ajax({
    					url: '<%=basePath%>im/music/doAdd.do?account='+acc,
    					type: 'post',
    					dataType: 'json',
    					data:rows[0],
    					success: function(data){
...
function delMovie(){
	...
	console.log(ids);
	var action='<%=basePath%>im/music/delete.do';
