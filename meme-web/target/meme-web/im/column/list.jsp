...
<table id="grid">
	<thead>
        <tr>
            <th field="id" width="15%" align="left">栏目编号</th>
            <th field="columnname" width="20%" align="left">栏目</th>
            <th field="columncode" width="10%" align="center">栏目编码</th>
...
function add(){
	addByCallback($('#grid'),$('#win'),'<%=basePath%>im/column/edit.do','<%=basePath%>im/column/doAdd.do',function(){
...
function edit(){
	...
	modifyByCallback($('#grid'),$('#win'),'<%=basePath%>im/column/edit.do','<%=basePath%>im/column/doEdit.do',function(){
		$('#grid').treegrid('reload');
	});
}
function del(){
	...
	$.messager.confirm('提示', '再次确认是否删除选中记录？', function(state){
		if (state){
			$.ajax({
				url: '<%=basePath%>im/column/delete.do',
				type:'post',
				dataType:'json',
				data:{'ids':ids},
...
$(function(){
	$('#grid').treegrid({
        url: '<%=basePath%>im/column/list.do',
        idField:'id',
