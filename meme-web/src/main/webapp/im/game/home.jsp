...
<div id="toolbar">
	<div class="express-search">
		<form id="searchForm" name="searchForm">
			<express:SelectTag id="sectioncode" type="select" dictGroupCode="GAME_COL" defaultOption="栏目分块类型" cssClass="easyui-combobox"/>
			&nbsp;&nbsp;
	...
	<div style="">
		<a href="javascript:add($('#grid'),$('#win'),'<%=basePath%>im/game/editView.do?columnid=${columnid}','<%=basePath%>im/game/add.do',{'title':'新增'});" class="easyui-linkbutton" iconCls="icon-add">增加</a>
		<a href="javascript:modify($('#grid'),$('#win'),'<%=basePath%>im/game/editView.do','<%=basePath%>im/game/edit.do',{'title':'修改'})" class="easyui-linkbutton" iconCls="icon-edit">修改</a>
		<a href="javascript:del($('#grid'),'<%=basePath%>im/game/delete.do')" class="easyui-linkbutton" iconCls="icon-remove">删除</a>
	</div>
</div>
<div id="win">
...
$(function(){
	$('#grid').datagrid({
		method: 'post',
        url: '<%=basePath%>im/game/homelist.do?columnid=${columnid}',
        width:'auto',
        height:'auto',
        rownumbers: true,
        fit: true,
