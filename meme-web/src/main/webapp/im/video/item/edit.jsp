	...
	<div class="panel panel-primary" style="margin-top:20px;">
		...
		<div class="panel-body">
			...
			<input type="hidden" name="itemid" id="itemid" value="${object.itemid}"/>
				...
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:10px;">视频所属分类:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						<select id="videoid" name="videoid" class="form-control">
						</select>
						...
						$.ajax({
							...
							success:function(data){
								if(data){
									...
									$('#videoid').append(options);
									$('#videoid').val('${object.videoid}');
								}
							},
							error:function(){
				...
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:10px;">视频地址:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						...
						<div class="checkbox">
							<label>
								<input id="uploadVideoBtn" type="checkbox">填写视频地址
							</label>
						</div>
					</div>
				...
				$(function(){
					...
					var videoUploadOption={
                        ...
                        filters: {
                        	mime_types : [
                        		{ title : "视频", extensions : "avi,wma,rmvb,rm,flash,mp4,mid,3gp" }
                        	],
                        	prevent_duplicates : true
                        },
                        init: {
                            ...
                            'BeforeUpload': function(up, file) {
                              ...
                              'UploadComplete': function() {
                                console.debug('视频文件上传完成');
                              },
                              'FileUploaded': function(up, file, info) {
                                var progress = new FileProgress(file, 'fsUploadProgress2');
                                console.debug("视频文件上传成功响应数据:",info.response);
                                progress.setComplete(up, info.response);
                                
                                $('#uploadDetail2').dialog('close');
                                var res = $.parseJSON(info.response);
                                var url=up.getOption('domain')+'/'+res.key;
                                console.debug('====视频文件地址：',url);
                                $('#url').val(url);
                              },
                              'Error': function(up, err, errTip) {
                            	  $('#uploadDetail2').dialog({
					...
					$('#uploadVideoBtn').click(function(){
						var url=$('input[name="url"]').val();
		          		var check=$(this).is(':checked');
		          		if(check){
		          			$('#videoContainer1').css('display','none');
		          			$('#videoContainer2').css('display','');
		          		}else{
		          			$('#videoContainer1').css('display','');
				...
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:20px;">最新大片:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						<express:SelectTag id="isnew" name="isnew" selected="${object.isnew}" type="select" dictGroupCode="SF" defaultOption="最新大片" cssClass="form-control"/>
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:20px;">本站推荐:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						<express:SelectTag id="isrecommend" name="isrecommend" selected="${object.isrecommend}" type="select" dictGroupCode="SF" defaultOption="本站推荐" cssClass="form-control"/>
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:20px;">主播自媒体:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						<express:SelectTag id="isanchor" name="isanchor" selected="${object.isanchor}" type="select" dictGroupCode="SF" defaultOption="主播自媒体" cssClass="form-control"/>
					</div>
				</div>
			</form>
		</div>
...
<script type="text/javascript" src="<%=basePath%>im/video/item/loadValidateRules.do"></script>
