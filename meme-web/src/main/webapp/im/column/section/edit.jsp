		...
		<div class="panel-body">
			...
			<input type="hidden" name="id" id="id" value="${object.id}"/>
				<div class="form-group">
					<label class="col-xs-3 control-label">栏目块名称:</label>
					<div class="col-xs-9">
						<input id="sectionname" name="sectionname" type="text" value="${object.sectionname}" class="form-control"/>
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:10px;">栏目块编码:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						<input id="sectioncode" name="sectioncode" type="text" value="${object.sectioncode}" class="form-control">
						<p class="help-block">栏目块编码要唯一，不能重复</p>
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:10px;">上级栏目块:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						<select id="pid" name="pid" class="form-control">
						</select>
						<script type="text/javascript">
						$.ajax({
							url: '<%=basePath%>im/column/section/getTree.do',
							type:'post',
							dataType:'json',
							success:function(data){
								if(data){
									var options=createTreeSelect(data,'sectionname','id','顶级栏目块');
									$('#pid').append(options);
									$('#pid').val('${object.pid}');
								}
							},
						...
						$('#pid').change(function(){
							...
							if(pid){
								$.ajax({
									url: '<%=basePath%>im/column/section/getSectionColumn.do',
									type:'post',
									dataType:'json',
									data: {'id':pid},
									success:function(data){
										if(data.state==1){
											var columnid=data.data.id;
											$('#columnid').val(columnid);
											$('#columnid').attr('disabled','true');
										}
									},
									error:function(){
										$.messager.alert('提示','操作失败','error');
				...
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:10px;">所属栏目:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						...
						$.ajax({
							...
							success:function(data){
								if(data){
									var options=createTreeSelect(data,'columnname','id','顶级栏目');
									$('#columnid').append(options);
									$('#columnid').val('${object.columnid}');
								}
							},
							error:function(){
								$.messager.alert('提示','操作失败','error');
				...
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:20px;">接口地址:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						<input id="sourceapi" name="sourceapi" type="text" value="${object.sourceapi}" class="form-control">
						<p class="help-block">如需另外配置接口地址，在此填写</p>
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:20px;">更多按钮链接地址:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						<input id="moreurl" name="moreurl" type="text" value="${object.moreurl}" class="form-control">
						<p class="help-block">如需另外更多按钮链接地址，在此填写</p>
					</div>
				</div>
				...
				<div class="form-group">
					<label class="col-xs-3 control-label" style="padding-top:20px;">是否分页:</label>
					<div class="col-xs-9" style="padding-top:10px;">
						<express:SelectTag id="ispagination" name="ispagination" selected="${object.ispagination}" type="select" dictGroupCode="SF" defaultOption="是否分页" cssClass="form-control"/>
					</div>
				</div>
...
<script type="text/javascript" src="<%=basePath%>im/column/section/loadValidateRules.do"></script>
