...
<table id="grid" toolbar="#toolbar">
	<thead>
		<tr>
			...
			<th field="itemname" sortable="true" align="center" width="10%">音乐名称</th>
			...
			<th field="url" sortable="false" align="center" width="10%">音乐地址</th>
			<th field="sortno" sortable="true" align="center" width="10%">排序号</th>
			<th field="reco_songText" sortable="false" align="center" width="10%">推荐歌曲</th>
			<th field="reco_mvText" sortable="false" align="center" width="10%">推荐MV</th>
			<th field="reco_audioText" sortable="false" align="center" width="10%">有声推荐</th>
...
<div id="toolbar">
	<div class="express-search">
		<form id="searchForm" name="searchForm">
			...
			&nbsp;&nbsp;
			<express:SelectTag id="reco_song" type="select" dictGroupCode="SF" defaultOption="推荐歌曲" cssClass="easyui-combobox"/>
			&nbsp;&nbsp;
			<express:SelectTag id="reco_mv" type="select" dictGroupCode="SF" defaultOption="推荐MV" cssClass="easyui-combobox"/>
			&nbsp;&nbsp;
			<express:SelectTag id="reco_audio" type="select" dictGroupCode="SF" defaultOption="有声推荐" cssClass="easyui-combobox"/>
			&nbsp;&nbsp;
	...
	<div style="">
		<a href="#" onclick="add($('#grid'),$('#win'),'<%=basePath%>im/audio/item/edit.do','<%=basePath%>im/audio/item/doAdd.do')" class="easyui-linkbutton" iconCls="icon-add">增加</a>
		<a href="#" onclick="modify($('#grid'),$('#win'),'<%=basePath%>im/audio/item/edit.do','<%=basePath%>im/audio/item/doEdit.do')" class="easyui-linkbutton" iconCls="icon-edit">修改</a>
...
function del(){
	...
	var action='<%=basePath%>im/audio/item/delete.do';
...
function modify(){
	var url="<%=basePath%>im/audio/item/edit.do";
	var action="<%=basePath%>im/audio/item/doEdit.do";
	var winObj=$('#win');
	var gridObj=$('#grid');
	var checkeds=gridObj.datagrid('getChecked');
	...
	winObj.dialog({
	    title: '编辑音乐：'+checkeds[0].itemname,
	    width: 800,
	    height: 600,
	    closed: false,
	    modal: true,
...
$(function(){
	$('#grid').datagrid({
		method: 'post',
        url: '<%=basePath%>im/audio/item/list.do',
        width:'auto',
        height:'auto',
        rownumbers: true,
        fit: true,
