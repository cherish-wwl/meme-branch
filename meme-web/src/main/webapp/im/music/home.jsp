...
<table id="grid" toolbar="#toolbar">
	<thead>
		<tr>
			...
			<th field="contentname" sortable="true" align="center" width="13%">音乐名称</th>
...
<div id="toolbar">
	<div class="express-search">
		<form id="searchForm" name="searchForm">
			<express:SelectTag id="sectioncode" type="select" dictGroupCode="AUDIO_COL" defaultOption="栏目分块类型" cssClass="easyui-combobox"/>
			&nbsp;&nbsp;
	...
	<div style="">
		<a href="javascript:add($('#grid'),$('#win'),'<%=basePath%>im/music/editView.do?columnid=${columnid}','<%=basePath%>im/music/add.do',{'title':'新增音乐'});" class="easyui-linkbutton" iconCls="icon-add">增加</a>
		<a href="javascript:modify($('#grid'),$('#win'),'<%=basePath%>im/music/editView.do','<%=basePath%>im/music/edit.do',{'title':'修改音乐'})" class="easyui-linkbutton" iconCls="icon-edit">修改</a>
		<a href="javascript:del($('#grid'),'<%=basePath%>im/music/delete.do')" class="easyui-linkbutton" iconCls="icon-remove">删除</a>
	</div>
</div>
<div id="win">
...
$(function(){
	$('#grid').datagrid({
		method: 'post',
        url: '<%=basePath%>im/music/homelist.do?columnid=${columnid}',
        width:'auto',
        height:'auto',
        rownumbers: true,
        fit: true,
