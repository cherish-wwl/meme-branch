            ...
            <div id="body" style="clear:left">
                <dl class="content">
                    ...
                    $('#payBtn').click(function(){
                    	$.ajax({
                    		url: '<%=basePath%>wechat/pay/h5/pay.do',
                    		type:'post',
                    		dataType:'json',
                    		data:$('form').serialize(),
                    		beforeSend: function(){
                    		...
                    		success:function(data){
                    			...
                    			if(data.state==1){
                    				...
                    				weui_dialog(data.message,function(){
                    					var url=window.location.href;
                    					window.location.href='<%=basePath%>member/clogin?redirect='+encodeURI(url);
                    				});
                    			}else{
                    				weui_alert(data.message);
                    			}
                    		},
                    		error:function(){
                    			weui_alert('操作异常');
