...
<body class="easyui-layout">
	...
	<div id="operationContainer" data-options="region:'east',split:true,collapsible:false" title="操作管理" style="width:20%;">
...
function switchPlatform(pfid){
	var params={'platformid':pfid,'organid':'${organid}'};
	loadMenuSuccess=false;
	loadOperationSuccess=false;
	loadMenuTree($('#menuList'),$('#m_menu'),'<%=basePath%>system/organization/getOrganMenus.do',params);
	loadOperation($('#operationList'),'<%=basePath%>system/organization/getOrganOperations.do',null);
...
function loadMenuTree(menuListObj,rclickMenuObj,url,params){
	menuListObj.tree({
	    ...
	    onSelect:function(node){
	    	...
	    	var params={'menuid':node.id,'organid':'${organid}'};
	    	loadOperationSuccess=false;
	    	loadOperation($('#operationList'),'<%=basePath%>system/organization/getOrganOperations.do',params);
	    },
	    onLoadSuccess:function(node, data){
	    	if(data&&data.length>0){
...
$(function(){
	...
	$.ajax({
		url:'<%=basePath%>system/role/initAuthorization.do?roleid=${roleid}',
		type:'post',
		dataType:'json',
		...
		error:function(){
			$.messager.alert('提示','获取角色授权功能操作失败，请检查异常日志','error');
		}
	});
	
	$.ajax({
		url:'<%=basePath%>system/organization/getOrganPlatforms.do',
		type:'post',
		dataType:'json',
		data:{'organid':'${organid}'},
		success:function(data){
			...
			$.each(data,function(i,ele){
				...
				if(i==0){
					firstPlatid=ele.id;
					var params={'platformid':ele.id,'organid':'${organid}'};
					loadMenuTree($('#menuList'),$('#m_menu'),'<%=basePath%>system/organization/getOrganMenus.do',params);
