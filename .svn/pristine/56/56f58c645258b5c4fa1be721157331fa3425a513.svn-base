...
import com.meme.core.excel.handler.ExcelHandlerFactory;
import com.meme.core.excel.handler.IExcelHandler;
...
public class ExcelManager<T>{
	 ...
	 */
	public void export(HttpServletRequest request,String tableName,Collection<T> list,OutputStream out) throws IllegalArgumentException, IllegalAccessException, IOException, ParseException, ClassNotFoundException, InstantiationException{
	...
	private void init(){
		...
		Document doc = null;
		try {
			doc = builder.read(new File(this.getClass().getResource("/").getPath()+"/template.xml"));
