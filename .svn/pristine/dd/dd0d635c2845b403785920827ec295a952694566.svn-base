...
<table id="grid">
	<thead>
        <tr>
            <th field="catname" width="20%" align="left">视频分类</th>
...
function add(){
	addByCallback($('#grid'),$('#win'),'<%=basePath%>im/video/category/editView.do','<%=basePath%>im/video/category/add.do',function(){
...
function edit(){
	...
	modifyByCallback($('#grid'),$('#win'),'<%=basePath%>im/video/category/editView.do','<%=basePath%>im/video/category/edit.do',function(){
		$('#grid').treegrid('reload');
	});
}
function del(){
	...
	$.messager.confirm('提示', '再次确认是否删除选中记录？', function(state){
		if (state){
			$.ajax({
				url: '<%=basePath%>im/video/category/delete.do',
				type:'post',
				dataType:'json',
				data:{'ids':ids},
...
$(function(){
	$('#grid').treegrid({
        url: '<%=basePath%>im/video/category/list.do',
        idField:'id',
        treeField:'catname'
	});
});
